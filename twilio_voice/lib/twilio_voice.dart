...
class TwilioVoice {
  ...
  CallEvent _parseCallEvent(String state) {
    ...
    } else if (state.startsWith("Call_Error|")){
      List<String> error = state.split('|');
      print(error[1]);
      call._activeCall =
          createCallFromState(state, callDirection: CallDirection.outgoing, error: error[1]);
      return CallEvent.callError;
    }
    switch (state) {
      case 'Ringing':
        return CallEvent.ringing;
...
ActiveCall createCallFromState(String state,
    {CallDirection? callDirection, bool initiated = false, String? error}) {
  List<String> tokens = state.split('|');
  if(state.startsWith('Call_Error')){
    return ActiveCall(
      from: '+0000',
      to: '+0000',
