        ...
        if (intent != null && intent.getAction() != null) {
            ...
            switch (action) {
                ...
                case Constants.ACTION_REJECT:
                    Log.e(TAG, "REJECT CALLLEDD ----");
                    Intent rejectCallI = new Intent("ACTIVITY_CALL_REJECT");
//                    rejectCallI.setAction("ACTIVITY_CALL_REJECT");
                    LocalBroadcastManager.getInstance(context).sendBroadcast(rejectCallI);
                    handleReject();
                    break;
                case Constants.ACTION_ACCEPT:
                    Log.e(TAG, "ACCEPT CALLLEDD ----");
                    int acceptOrigin = intent.getIntExtra(Constants.ACCEPT_CALL_ORIGIN, 0);
                    if (acceptOrigin == 0) {
                        Intent acceptCall = new Intent("ACTIVITY_CALL_ACCEPT");
                        LocalBroadcastManager.getInstance(context).sendBroadcast(acceptCall);
                    } else {
                        answer();
                    }
                    break;
    ...
    private Call.Listener callListener() {
        return new Call.Listener() {
            ...
            public void onConnectFailure(Call call, CallException error) {
                ...
                Log.e(TAG, message);
                sendPhoneCallEvents("Call_Error|" + message);
